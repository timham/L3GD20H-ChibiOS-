...
//extern void  GPIO_Init(GPIO_Type*  GPIOx, GPIO_InitTydeDef*  GPIO_InitStruct);
...
static void SPI_RxFIFOThresholdConfig( SPI_TypeDef* SPIx, 
											  uint16_t SPI_RxFIFOThreshold, SPIConfig *scfg)
{
  /* Clear FRXTH bit */
...
void L3GD20_Init(L3GD20_InitTypeDef *L3GD20_InitStruct, SPIDriver *lspidrive)
{	
	uint8_t ctrl1 = 0x00, ctrl4 = 0x00;

	lspidrive->spi = SPI1;
	L3GD20_LowLevelInit(lspidrive);
	...
	//palWritePort(GPIOE,bits)

	/* Write value to MEMS CTRL_REG1 regsister */
	L3GD20_Write(&ctrl1, 0x20, 1);

	/* Write value to MEMS CTRL_REG4 regsister */
	L3GD20_Write(&ctrl4, 0x23, 1);
...
void  L3GD20_LowLevelInit(SPIDriver *drvspi)
{
...
#if 0	// ??? According to the st platform driver, this performed three times.
...
/*
	spicfg.cr1 = drvspi->config.cr1;
	spicfg.cr2 = drvspi->config.cr2;
	spicfg->end_cb = NULL;
	spicfg.sspad = 0x0080;
	spicfg->ssport = GPIOE;
*/	
#if 0
	drvspi->config->end_cb = NULL;
	drvspi->config->ssport = GPIOE;    // ?????
	drvspi->config.sspad = 0x0080;    // ?????
	drvspi->config.cr1 = drvspi->spi.CR1;
	drvspi->config.cr2 = drvspi->spi.CR2;
#endif

	//////////////////////////////////////////////////////////////////////////
	
	_pal_lld_setgroupmode(GPIOE, PAL_STM32_MODE_MASK, 0x02);  // GPIO_Mode_OUT
	_pal_lld_setgroupmode(GPIOE, PAL_STM32_OTYPE_MASK, 0x00); // 3: GPIO_OType_PP
	_pal_lld_setgroupmode(GPIOE, PAL_STM32_OSPEED_HIGHEST, 0x03);

	pal_lld_writepad(GPIOE,3, GPIO_BSRR_BS_3);
	pal_lld_setport(GPIOE, GPIO_BSRR_BS_3);

	_pal_lld_setgroupmode(GPIOE, PAL_STM32_MODE_MASK,0x00);   // GPIO_Mode_IN
	_pal_lld_setgroupmode(GPIOE, PAL_STM32_OTYPE_MASK, 0x00);
	_pal_lld_setgroupmode(GPIOE, PAL_STM32_OSPEED_HIGHEST, 0x03);
	_pal_lld_setgroupmode(GPIOE, PAL_STM32_OTYPE_PUSHPULL, 0x00);
